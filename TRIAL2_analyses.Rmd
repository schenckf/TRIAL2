...
#b = transmission rate
#mi = infected leaf mortality rate (no healthy leaf mortality rate because leaves always infected before mortality)
...
cost_function <- function(x, times, N0, data) {
  ...
  mi <- x[3]
  if (a > 0 & b > 0 & mi > 0) {
    sim <-ode(y = N0, t = times, func = solve.Disease.SA, parms = list(a = x[1], b = x[2], mi = x[3]), method = "ode45")[c(1,2,3,4,5,6,8,11,15,22,29),]
    if (NROW(sim) == NROW(data)) {
      rmse <- sqrt(mean((as.numeric(data[,2] - (sim[,2])))^2)) + sqrt(mean((as.numeric(data[,3] - (sim[,3])))^2)) + sqrt(mean((as.numeric(data[,4] - (sim[,4])))^2))
    } else {
      rmse <- Inf
    }
  } else {
...
sim_annealing <- function(I, tc, tr, data, times, N0) {
  rmse <- numeric(length = I); pars <- matrix(nrow = I, ncol = 3)
  colnames(pars) <- c("a", "b", "mi")
  x_curr <- c(runif(1, min = 0, max = 1), runif(1, min = 0, max = 1), runif(1, min = 0, max = 1))
  rmse_curr <- cost_function(x_curr, times, N0, data)
  
  for (i in 1:I) {
    temp <- tc * exp(- tr * i / I)
    x_prop <- c(runif(1, min = 0, max = 1), runif(1, min = 0, max = 1), runif(1, min = 0, max = 1))
...
##Running Simulations
#set.seed(1)
data <- mesocosm.transmission.LP
times <- 0:28
N0 <- c(S = 2.58, I = 1, D = 0) #averages from eelgrass at 0 time point of experiment
...
sim.final <-ode(y = N0, t = times, func = solve.Disease.SA, parms = list(a = 0.076, b = 0.213, mi = 0.035), method = "ode45")[c(1,2,3,4,5,6,8,11,15,22,29),]

## Plotting data
par(mfrow = c(1,1))
plot(mean.s ~ Day, data = mesocosm.transmission.LP, col = "green", ylim = c(0,5)) #Experiment susceptible
points(mean.i ~ Day, data = mesocosm.transmission.LP, col = "red") #Experiment infected
points(mean.d ~ Day, data = mesocosm.transmission.LP) #Experiment dead
