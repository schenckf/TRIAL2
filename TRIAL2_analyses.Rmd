...
m.new.shoots <- aov(shoot.production ~ site*infection, data = mesocosm.shoot)
...
shapiro.test(residuals(m.new.leaves))
leveneTest(m.new.leaves)
...
shapiro.test(residuals(m.lost.leaves))
leveneTest(m.lost.leaves)
...
m.prevalence <- glmer(cbind(lesion.present, lesion.absent) ~ site + (1|site/quad.unique), family = binomial, data = filter(field.leaf, year == "2016"))
...
m.Dis1 <- glmer(cbind(lesion.present, lesion.absent) ~ length.scale + (1|site), family = binomial, data = field.leaf.2016)
summary(m.Dis1)

m.Dis2 <- glmer(cbind(lesion.present, lesion.absent) ~ density.scale + (1|site), family = binomial, data = field.leaf.2016)
summary(m.Dis2)

m.Dis3 <- glmer(cbind(lesion.present, lesion.absent) ~ length.scale + density.scale + (1|site), family = binomial, data = field.leaf.2016)
summary(m.Dis3)
...
field.quad <- field.leaf %>%
  filter(year == "2016") %>%
  group_by(site, quad) %>%
  summarize(shoot.m2 = mean(shoot.m2, na.rm = T), prevalence = sum(lesion.present)/(sum(lesion.present) + sum(lesion.absent)))
...
m.density <- aov(shoot.m2 ~ site*prevalence, data = field.quad)
...
m.length <- aov(length.cm ~ site*lesion.present.factor + site/quad, data = field.leaf.2016)
summary(m.length) #significant effects of site and disease status (need to fix model to have site/quad as random effect?)
anova(m.length)

posthoc.length <- TukeyHSD(aov(length.cm ~ site*lesion.present.factor, data = field.leaf.2016))
posthoc.length
multcompLetters(posthoc.length$`site`[,"p adj"])

#summary statistics
field.length.summary <- field.leaf.2016 %>%
  group_by(lesion.present.factor) %>%
