...
multcompLetters(posthoc.SA.ms.model$`site`[,"p adj"]) #nb = least, lp&cb = ND, wb = most
...
solve.field <- function(t, y, parms) {
  with(c(parms, y), {
    ...
    dI <- beta * S * I - mi * (I / (S + I)) * ((S + I) / lpp)
    #dD <- ms * (S / (S + I)) * ((S + I) / lpp) + mi * (I / (S + I)) * ((S + I) / lpp)
    return(list(c(dS, dI))) #dD
  })
}
...
beta <- c(0.295*0.001, 0.341*0.001, 0.434*0.001, 0.547*0.001)
ms <- c(0.042, 0.029, 0.019, 0.060)
mi <- c(0.098, 0.108, 0.089, 0.060)

#empty matrix
r.T1 <- matrix(nrow = 10001, ncol = 3, 0)

#time steps
times <- seq(from = 0, to = 10000, len = 10001)

#simulation (can adjust parameters for each site see oyster modeling project)
r.T1 <- ode(time = times, parms = list(as = as[2], ai = ai[2], lpp = lpp[2], beta = beta[2], ms = ms[2], mi = mi[2]), y = c(S = 50, I = 1),  func = solve.field, method = "ode45")

#for (n in 1:4) {
  #r.T1[,1] <- ode(time = times, parms = list(as = as[1], ai = ai[1], lpp = lpp[1], beta = beta[1], ms = ms[1], mi = mi[1]), y = c(S = 750, I = 750),  func = solve.field, method = "ode45")[,1]
  
  #r.T1[,n+1] <- ode(time = times, parms = list(as = as[n], ai = ai[n], lpp = lpp[n], beta = beta[n], ms = ms[n], mi = mi[n]), y = c(S = 750, I = 750),  func = solve.field, method = "ode45")[,2]
  
  #r.T1[,n+5] <- ode(time = times, parms = list(as = as[n], ai = ai[n], lpp = lpp[n], beta = beta[n], ms = ms[n], mi = mi[n]), y = c(S = 750, I = 750),  func = solve.field, method = "ode45")[,3]
  #}
...
#parameters (DC, LP, NB, WB)
      ### ---> Simulation results aren't making a lot of sense (all becoming infected)?
          ###Possible causes: (1) transmission rates are too high? (2) Infected mortality rates too low?
