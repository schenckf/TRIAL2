...
mesocosm.shoot <- mesocosm.leaf %>%
  select(c(month, year, mesocosm, vase, infection, site, experiment.day, no.leaves, no.leaves.lost, no.leaves.new, shoot.no, S1.total.length, S2.total.length, rhizome.length, ag.biomass, bg.biomass, shoot.notes)) %>%
  group_by(month, year, mesocosm, vase, infection, site, experiment.day) %>%
  summarize(no.leaves = mean(no.leaves, na.rm = T), no.leaves.lost = mean(no.leaves.lost, na.rm = T), no.leaves.new = mean(no.leaves.new, na.rm = T), shoot.no = mean(shoot.no, na.rm = T), S1.total.length = mean(S1.total.length, na.rm = T), S2.total.length = mean(S2.total.length, na.rm = T), rhizome.length = mean(rhizome.length, na.rm = T), ag.biomass = mean(ag.biomass, na.rm = T), bg.biomass = mean(bg.biomass, na.rm = T))
...
mesocosm.shoot <-select(mesocosm.shoot.temp, c(month, year, mesocosm, vase, infection, site, no.leaves.initial, shoot.no.initial, shoot.no.final, S1.total.length.initial, S2.total.length.final, rhizome.length.initial, rhizome.length.final, ag.biomass.final, bg.biomass.final))

mesocosm.shoot.leaf.no.final.edit <- select(mesocosm.shoot.temp, c(month:site, no.leaves.final, no.leaves.lost.final, no.leaves.new.final, S1.total.length.final, S2.total.length.final, shoot.no.final))
mesocosm.shoot.leaf.no.final.edit <- subset(mesocosm.shoot.leaf.no.final.edit, shoot.no.final != 0)
mesocosm.shoot.leaf.no.final.edit <- select(mesocosm.shoot.leaf.no.final.edit, c(month:S1.total.length.final)) #removing final leaf number values of dead shoots
...
field.quad <- field.leaf %>%
  filter(year == "2016") %>%
  group_by(site, quad) %>%
  summarize(density.decm = mean(shoot.m2, na.rm = T)/100, canopy.height = mean(length.cm, na.rm = T), prevalence = sum(lesion.present)/(sum(lesion.present) + sum(lesion.absent)))
...
prop.new.shoot <- subset(mesocosm.shoot, site == "LP")
...
anova(m.lost.leaves) #significant effect of infection (# of leaves intitial included as a covariate)
...
###Density
```{r}
p <- ggplot(field.quad, aes(x = site, y = shoot.m2)) + 
  #scale_fill_manual(values = c("white", "gray")) +
  geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 3, color = "red", position = position_dodge(1)) +
...
plot(shoot.m2 ~ prevalence, field.quad)
...
###leaf number
```{r}
m.leaf.number <- lmer(no.leaves ~ site + (1|quad.unique), data = field.leaf.2016)
...
times <- seq(from = 0, to = 100, len = 101)
...
ct = c(0.0136728, 0.0123120, 0.0123120, 0.01443714) #contacts per time between infected and susceptible leaves (FIELD, density & length) *divided by 1*10^7 (should I account for density in this metric?? maybe no? ... yes I think I should account for compactness of shoots)
            #DC, LP, NB length
              #(78.61+66.65+70.74)/3 #= 72
              #DC length*leaf.no*density = 149280.4 (site specific leaf.no)
              #LP length*leaf.no*density = 
              #NB length*leaf.no*density = 
            #WB length = 104.39
              #WB length*leaf.no*density = 144371.4
...
r.T1 <- matrix(nrow = 101, ncol = 9, 0)

#simulation (can adjust parameters for each site see oyster modeling project)
for (n in 1:4) {
  r.T1[,1] <- ode(time = times, parms = list(a = a[1], ai = ai[1], cc = cc[1], ic = ic[1], ct = ct[1], m = m[1], mi = mi[1]), y = c(S = 100, I = 1),  func = solve.Disease.M, method = "ode45")[,1]
  r.T1[,n+1] <- ode(time = times, parms = list(a = a[n], ai = ai[n], cc = cc[n], ic = ic[n], ct = ct[n], m = m[n], mi = mi[n]), y = c(S = 10000, I = 1),  func = solve.Disease.M, method = "ode45")[,2]
  r.T1[,n+5] <- ode(time = times, parms = list(a = a[n], ai = ai[n], cc = cc[n], ic = ic[n], ct = ct[n], m = m[n], mi = mi[n]), y = c(S = 10000, I = 1),  func = solve.Disease.M, method = "ode45")[,3]
  }

r.T1 <- data.frame(r.T1)

prevalence <-c(r.T1[101,6]/(r.T1[101,2] + r.T1[101,6]), r.T1[101,7]/(r.T1[101,3] + r.T1[101,7]), r.T1[101,8]/(r.T1[101,4] + r.T1[101,8]), r.T1[101,9]/(r.T1[101,5] + r.T1[101,9]))
prevalence

plot((r.T1[,3]/(r.T1[,3] + r.T1[,2])) ~ r.T1[,1], t = "l",
     ylim = c(0,1), ylab = "prevalence", xlab = "time", main = "LP")
plot(r.T1[,2] ~ r.T1[,1], t = "l", ylim = c(0,3000), ylab = "number of individuals", xlab = "time", col = "green", main = "LP")
lines(r.T1[,1], r.T1[,3], t = "l", col = "red")
