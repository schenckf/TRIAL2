...
field.site.mean$transmission <- c(0.77, 0.97, 0.89, 0.81)
...
##Leaf loss
```{r eval = FALSE, echo = FALSE}
p <- ggplot(filter(mesocosm.shoot, vase != 17 & vase != 21 & vase != 27 & vase != 29 & vase != 37 & vase != 47 & vase != 59), aes(x = site, y = no.leaves.lost.final, fill = infection)) + 
  scale_fill_manual(values = c("white", "gray")) +
  geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 3, color = "red", position = position_dodge(1)) +
...
m.ewdss.risk <- glm(cbind(lesion.presence, lesion.absence) ~ field.prevalence * infection + exp.day.appeared, family = binomial, data = filter(mesocosm.lesions))
...
## transmission
m.t.p <- glm(cbind(lesion.present, lesion.absent) ~ transmission, family = binomial, data = field.site.mean)
Anova(m.t.p, test = "LR")

m.t.p <- lm(p.mean ~ transmission, data = field.site.mean)
Anova(m.t.p)

p <- ggplot(field.site.mean, aes(x = transmission, y = prevalence)) +
  #scale_color_manual(values = c("gray40","gray60", "gray90", "black")) +
  geom_point() +
  geom_smooth(method = lm, se = F, fullrange = F, color = "black") +
  theme_classic()
...
#as = per healthy leaf, leaf production rate (currently leaf production leaves/time)
#ai = per infected leaf, leaf production rate (currently ommitted)
#b = transmission rate
...
cost_function <- function(x, times, N0, data) {
  ...
  mi <- x[3]
  ai <- x[4]
  if (as > 0 & b > 0 & mi > 0 & ai > 0) {  #
    sim <-ode(y = N0, t = times, func = solve.Disease.SA,
              parms = list(as = x[1], b = x[2], mi = x[3], ai = x[4]), #
...
sim_annealing <- function(I, tc, tr, data, times, N0) {
  rmse <- numeric(length = I); pars <- matrix(nrow = I, ncol = 4) #
  colnames(pars) <- c("as", "b", "mi", "ai") #, "ai"
  x_curr <- c(runif(1, min = 0, max = 0.01), runif(1, min = 0, max = 1),
              runif(1, min = 0, max = 1), runif(1, min = 0, max = 0.01)) #
  rmse_curr <- cost_function(x_curr, times, N0, data)
  
  for (i in 1:I) {
    temp <- tc * exp(- tr * i / I)
    x_prop <- c(runif(1, min = 0, max = 0.01), runif(1, min = 0, max = 1),
                runif(1, min = 0, max = 1), runif(1, min = 0, max = 0.01)) #
...
sim.final <-ode(y = N0, t = times, func = solve.Disease.SA,
                parms = list(as = 0.0077, b = 0.2058, mi = 0.0185, ai = 0.0069),
                method = "ode45")[c(1,2,3,4,5,6,8,11,15,22,29),]
...
###multiple data sets
set.seed(1)
pars.sim <- matrix(NA, nrow = 11, ncol = 4) #
for (i in 1:11) {
...
sim.final <- array(NA, c(11,4,11), )
for (i in 1:11) {
  sim.final[, , i] <-ode(y = N0, t = times, func = solve.Disease.SA, parms = list(as = pars.sim[i, 1], b = pars.sim[i, 2], mi = pars.sim[i, 3], ai = pars.sim[i, 4]), method = "ode45")[c(1,2,3,4,5,6,8,11,15,22,29),] #, ai = pars.sim[i,2]
}

sim.final

par(mfrow = c(1,1))
  plot(mesocosm.transmission.CB[, 2] ~ mesocosm.transmission.CB[, 1], data = mesocosm.transmission.CB, col = "green", ylim = c(0,5)) #Experiment susceptible
  for (i in 2:11) {
    points(mesocosm.transmission.CB[, i + 1] ~ mesocosm.transmission.CB[, 1], data = mesocosm.transmission.CB, col = "green")
  }
for (i in 1:11) {
  points(mesocosm.transmission.CB[, i + 12] ~ mesocosm.transmission.CB[, 1], data = mesocosm.transmission.CB, col = "red") #Experiment infected
    points(mesocosm.transmission.CB[, i + 23] ~ mesocosm.transmission.CB[, 1], data = mesocosm.transmission.CB) #Experiment dead
    points(sim.final[,2,i] ~ sim.final[, 1, i], data = sim.final[ , , i], t = "l", col = "green") #simulated susceptible
    points(sim.final[,3,i] ~ sim.final[, 1, i], data = sim.final[, , i], t = "l", col = "red") #simulated infected
    points(sim.final[,4,i] ~ sim.final[,1,i], data = sim.final[, , i], t = "l") #simulated dead
}
