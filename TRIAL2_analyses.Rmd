...
`11AAm.density <- aov(density.decm ~ site, data = field.quad)
...
solve.Disease.M <- function(t, y, parms) {
  with(c(parms, y), {
     ...
     dS <- (A * (S / (S + I)) + Ai * (I / (S + I))) - ((ic * ct) * S * I) - (M * (S / (S + I)))  ###SHOOT DEPENDENT LEAF GROWTH (assuming constant shoot number i.e. no shoot mortality)
     #dS <- ((a - m) * S + ai * I) - (ic * ct) * S * I ###LEAF DEPENDENT LEAF GROWTH
     dI <- ((ic *ct) * S * I) - (Mi * (I / (S + I)))
     #dI <- (ic * ct) * S * I - mi * I
...
M = c((0.81 / 28) * 461, (0.81 / 28) * 461, (0.81 / 28) * 461, (0.81 / 28) * 461) #mortality of leaves per 1 m2 quadrat per day from healthy shoots (MESOCOSM and FIELD)
#m = c(0.36, 0.18, 0.18, 0.28) #mortality rate of susceptible leaves per susceptible leaf (MESOCOSM)
        #DC healthy: 1.3/3.58 = 0.36; NB healthy: 0.64/3.58 = 0.18; LP healthy: 0.64/3.58 = 0.18; WB healthy: 0.73/2.64 = 0.28
Mi = c((1.95 / 28) * 461, (2.7 / 28) * 461, (1.95 / 28) * 461, (1.68 / 28) * 461) #mortality of leaves per 1 m2 quadrat per day from diseased shoots (MESOCOSM AND FIELD)
...
for (n in 1:4) {
  r.T1[,1] <- ode(time = times, parms = list(A = A[1], Ai = Ai[1], ic = ic[1], ct = ct[1], M = M[1], Mi = Mi[1]), y = c(S = 1000, I = 1),  func = solve.Disease.M, method = "ode45")[,1]
  
  r.T1[,n+1] <- ode(time = times, parms = list(A = A[n], Ai = Ai[n], ic = ic[n], ct = ct[n], M = M[n], Mi = Mi[n]), y = c(S = 1000, I = 1),  func = solve.Disease.M, method = "ode45")[,2]
  
  r.T1[,n+5] <- ode(time = times, parms = list(A = A[n], Ai = Ai[n], ic = ic[n], ct = ct[n], M = M[n], Mi = Mi[n]), y = c(S = 1000, I = 1),  func = solve.Disease.M, method = "ode45")[,3]
  }
...
plot((r.T1[,7]/(r.T1[,3] + r.T1[,7])) ~ r.T1[,1], t = "l",
...
lines(r.T1[,1], r.T1[,7]/(r.T1[,3] + r.T1[,7]), t = "l", col = "red")
lines(r.T1[,1], r.T1[,8]/(r.T1[,4] + r.T1[,8]), t = "l", col = "blue")
lines(r.T1[,1], r.T1[,9]/(r.T1[,5] + r.T1[,9]), t = "l", col = "darkgreen")
