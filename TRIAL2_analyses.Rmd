...
mesocosm.lesions <- mesocosm.leaf %>%
  ...
  dplyr::select(c(month, year, mesocosm, vase, infection, site, experiment.day, S1.no.leaves, area.cm, lesion.area, lesion.pc, lesion.presence, lesion.absence, notes)) %>%
  group_by(month, year, mesocosm, vase, infection, site, experiment.day) %>%
  summarize(S1.no.leaves = mean(S1.no.leaves, na.rm = T), area.cm = sum(area.cm, na.rm = T), lesion.area = sum(lesion.area, na.rm = T), WSWI = mean(lesion.pc, na.rm = T), lesion.presence = sum(lesion.presence, na.rm = T), lesion.absence = sum(lesion.absence, na.rm = T))
...
```{r}
WSWI.model <- aov(lesions.pc ~ site * infection, data = filter(mesocosm.lesions, vase != "22" & vase != "24")) ##mesocosm not significant so removed from the model
Anova(WSWI.model)
...
```{r}
EWDSS.model <- glm(cbind(lesion.presence, lesion.absence) ~ site * infection + mesocosm, family = binomial, data = mesocosm.lesions)
Anova(EWDSS.model)
...
m.prevalence.quad <- aov(prevalence ~ canopy.height + density + site, data = field.quad) #density not significant so removed from model?
Anova(m.prevalence.quad)
