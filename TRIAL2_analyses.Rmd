...
###NEED TO CHANGE MODEL!!!! PER LEAF IS CONFUSING
solve.Disease.SA <- function(t, y, parms) {
  y[y < 0] <- 0
  with(c(parms, y), {
     y[y < 0] <- 0
     dS <- as - b * S * I # * S + ai * I (remove per leaf)
  ...
  for (i in 1:11) {
    N0 <- c(S = data.temp.sim[1, i + 1], I = 1, D = 0)
      sim.final[, , (i+(j*11))] <-ode(y = N0, t = times, func = solve.Disease.SA,
                             parms = list(as = pars.sim.master[i, 1, (j+1)],
...
rbind(means.cb, means.lp, means.nb, means.wb)
