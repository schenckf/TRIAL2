...
mesocosm.lesions <- mesocosm.leaf %>%
  ...
  dplyr::select(c(month, year, mesocosm, vase, infection, site, experiment.day, S1.no.leaves, lesion.pc, lesion.presence, lesion.absence, notes)) %>%
  group_by(month, year, mesocosm, vase, infection, site, experiment.day) %>%
  summarize(S1.no.leaves = mean(S1.no.leaves, na.rm = T), lesion.pc = mean(lesion.pc, na.rm = T), lesion.presence = sum(lesion.presence, na.rm = T), lesion.absence = sum(lesion.absence, na.rm = T))
...
##Leaf production
```{r}
p <- ggplot(mesocosm.shoot, aes(x = site, y = no.leaves.new.final, fill = infection)) + 
  scale_fill_manual(values = c("white", "gray")) +
  geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 3, color = "red", position = position_dodge(1)) +
...
m.new.leaves <- aov(no.leaves.new.final ~ site*infection + mesocosm + no.leaves.initial, data = mesocosm.shoot)
...
```{r}
#proportion of exp leaf 0 infected (leaves produced during exp.)
p <- ggplot(filter(mesocosm.leaf, experiment.day == "28" & infection == "Y" & shoot.no != 0 & exp.leaf.no != "1" & exp.leaf.no != "2" & exp.leaf.no != "3" & exp.leaf.no != "4" & exp.leaf.no != "5"), aes(x = field.prevalence, y = lesion.presence, fill = site)) + #& exp.leaf.no != "-2" & exp.leaf.no != "4" & exp.leaf.no != "5" 
  ...
  labs(x = "Field Prevalence", y = "lesion present") +
  ylim(0,1) +
  xlim (0,1) +
  theme_classic()
p

m.severity.L1 <- glm(cbind(lesion.presence, lesion.absence) ~ field.prevalence, family = binomial, data = filter(mesocosm.leaf, experiment.day == "28" & infection == "Y" & shoot.no != 0 & exp.leaf.no != "1" & exp.leaf.no != "2" & exp.leaf.no != "3" & exp.leaf.no != "4" & exp.leaf.no != "5"))
...
p <- ggplot(mesocosm.lesions, aes(x = site, y = lesion.presence/(lesion.presence+lesion.absence), fill = infection)) +
  ...
  labs(x = "site", y = "proportion of leaves infected") +
  ylim(0,1) +
  theme_classic()
p


m.severity <- glm(cbind(lesion.presence, lesion.absence) ~ site, family = binomial, data = filter(mesocosm.lesions, infection == "Y"))
...
p <- ggplot(mesocosm.lesions, aes(x = site, y = lesion.pc, fill = infection)) +
  scale_fill_manual(values = c("white", "gray")) +
  #geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 1, color = "red", position = position_dodge(1)) +
...
m.lesion.pc.d28.all <- aov(lesion.pc ~ infsite + mesocosm + shoot.ID, data = filter(mesocosm.leaf, experiment.day == "28" & exp.leaf.no !="-2" & exp.leaf.no != "4" & exp.leaf.no != "5")) ##Including leaves with 0% infection, exclude leaves 4 and 5 because of lack of replication across sites, lesion.pc.y not significant so removed from model
