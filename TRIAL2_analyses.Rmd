...
p <- ggplot(filter(mesocosm.leaf, experiment.day == "28" & infection == "Y" & shoot.no != 0), aes(x = current.leaf.no, y = lesion.presence, fill = site)) + #& exp.leaf.no != "-2" & exp.leaf.no != "4" & exp.leaf.no != "5" 
  ...
  labs(x = "current leaf no.", y = "lesion percent cover") +
  ylim(0,1) +
  theme_classic()
p
...
m.severity <- glm(cbind(lesion.presence, lesion.absence) ~ site, family = binomial, data = filter(mesocosm.lesions, infection == "N"))
