...
#uploading data
```{r Data Upload}
(setwd("~/Dropbox/EEMB Graduate/Research/ZosteraWastingDisease/Greenhouse_Mesocosm/Innoculation Trials/Trial_2/Analyses/Data for analyses"))

##Leaf morphology and % cover
d1 <- read.csv("T2_PRELIM_DATA_23JAN2017.csv")

str(d1)

d1$no. <- as.factor(d1$no.)
d1$experiment.day <- as.factor(d1$experiment.day)
d1$exp.leaf.no <- as.factor(d1$exp.leaf.no)
d1$current.leaf.no <- as.factor(d1$current.leaf.no)
d1$mesocosm <- as.factor(d1$mesocosm)
d1$vase <- as.factor(d1$vase)

d1$shoot.ID <- paste(d1$site, d1$no.)
d1$shoot.ID <- as.factor(d1$shoot.ID)

## Shoot and rhizome biomass
d1.shoot <- read.csv("T2_PRELIM_BIOMASS_DATA_19DEC2016.csv")
